...
template <class A> const A& min(const A& v1, const A& v2) {
	return v2 < v1 ? v2 : v1;
}

template <class A, class CMP> const A& min(const A& v1, const A& v2, const CMP& cmp) {
	return cmp(v2, v1) ? v2 : v1;
}

template <class A> const A& max(const A& v1, const A& v2) {
	return v1 < v2 ? v2 : v1;
}

template <class A, class CMP> const A& max(const A& v1, const A& v2, const CMP& cmp) {
